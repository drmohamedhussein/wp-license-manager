...
class WPLM_Advanced_Licensing {
    ...
    private function perform_enhanced_validation($params) {
        ...
        $license_post = $license_query->have_posts() ? $license_query->posts[0] : null;
        if (!$license_post) {
    ...
    private function deactivate_license_on_domain($license_key, $domain) {
        ...
        $result = $wpdb->query($wpdb->prepare(
            "UPDATE {$wpdb->postmeta} 
             ...
             END
             WHERE post_id = %d 
             AND meta_key IN ('_wplm_activated_domains', '_wplm_fingerprints', '_wplm_site_data')",
        ...
        if ($result !== false) {
            ...
            return ['valid' => true, 'message' => __('License lightning deactivated successfully', 'wp-license-manager')];
        } else {
            return ['valid' => false, 'message' => __('Deactivation failed', 'wp-license-manager')];
        }
    }
